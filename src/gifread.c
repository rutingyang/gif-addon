...
#define CUR_CODE_MASK ((1 << bits_needed) - 1)
                                   ...
                                   Gif_Reader *grr)
    {
        ...
        if (len == 4) {
            ...
            gfi->transparency = (packed & 1) != 0 ? 1 : 0;
    ...
    {
        // LOG_I("=== gifreader reader read_gif  begin ===");
        Gif_Stream *gfs;
        Gif_Image *gfi;
        Gif_Context gfc;
        ...
        if (gifgetc(grr) != 'G' ||
            gifgetc(grr) != 'I' ||
            gifgetc(grr) != 'F'){
            // LOG_I("=== gifreader reader read_gif  retun 0 ===");
            return 0;
        }
        (void)gifgetc(grr);
        (void)gifgetc(grr);
    ...
    {
        // LOG_I("=== gifreader reader Gif_FullReadFile begin ===");
